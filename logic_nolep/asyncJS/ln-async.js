...
function getUserDataCallback(userId, callback) {
    ...
    callback(data ? data : new Error("User not found"));
}

// Implementasi Promise
function getUserDataPromise(userId) {
    return new Promise(
        (resolve, reject) => getUserDataCallback(userId, (data, err) => err ? reject(err) : resolve(data))
    );
}
...
async function getUserDataAsync(userId) {
    try {
        const data = await getUserDataPromise(userId);
        return data;
    } catch (err) {
        throw err;
    }
