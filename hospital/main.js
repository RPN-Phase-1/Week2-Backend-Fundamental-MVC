...
Deskripsi:
Kalian akan membuat aplikasi backend sederhana menggunakan terminal interface. kita akan mengatur input command 
dengan process.argv, dan menyesuaikan command tersebut di function function controller.
...
index.js
File ini yang akan kita eksekusi di command , dan menangkap semua process argv dari comman yang ada, 
disini kita bakal handling condition setiap command. jadi ketika kita ketik:
...
setiap command hanya boleh melakukan logic di dalam file class controller, dan method method di controller ini 
static, jadi bisa di akses di index.js tanpa constructor.
...
/*
controller.js
file controller inilah yang akan kalian handle logic logic dari command yang di berikan, jika command register 
maka kalian akan melakukan logic insert data ke model. model ini wajib beruba class instance jadi seakan akan 
data ini mempunya schema. kalian tidak boleh mengubah JSON langsung di dalam controller. wajib lewat model.*/
...
ini file yang akan kalian pakai sebagai model, file model ini berfungsi untuk melakukan perubahan pada data json 
kita, jadi disini kalian akan sering read/write file json yang akan kita jadikan sebagai penyimpanan. 
setiap command WAJIB memakai asynchronous jadi bole pake callback, promise , atau async/await. disini gua 
contohin pake callback.
...
==============================================================================================
view.js
File ini berfungsi untuk memberikan visualisasi dari hasil logic controller kalian, apapun output yang ada 
di terminal wajib di handle sama file ini. kalian tidak boleh log di controller/model/index.js */

class HospitalView {
    static registerView(objArr) {
        console.log(`save data success {"username":${objArr[0].username},"password":${objArr[0].password},
        "role":${objArr[0].position}. Total employee : ${objArr[1]}`)
    }
